...
namespace B2b.Web.v4.Models.EntityLayer
{
    public class PosBankDetail : DataAccess
    {
        ...
        {
            PosBankDetail list = new PosBankDetail();
            DataTable dt = DAL.GetPosPaymentChoicePayBank(pOneShot, pBankId);

            foreach (DataRow row in dt.Rows)
            {
                PosBankDetail item = new PosBankDetail()
                {
                    Id = row.Field<int>("Id"),
        ...
        {
            PosBankDetail list = new PosBankDetail();
            DataTable dt = DAL.GetPosBankdEtailByPosBankId(pPosBankId);
