...
var myVar;
var currentSalesman = null;
function checkConnectTime() {
    if (currentSalesman == null) {
        fireCustomLoading(true);

        $.ajax({
            type: "POST",
            url: "/Admin/Home/GetSalesman",
            data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                fireCustomLoading(false);                
                currentSalesman = response;
                startConnectionTimer(currentSalesman);
            },
        });
    }
    else {
...
function startConnectionTimer(response) {
    if (response.IsAutoLock) {
        myVar = setTimeout(function () {
            resetConnectionTime();
            var autoResponseTime = setTimeout(function () { window.location.href = "/Admin/Login/Locked" }, 10000)
            $.confirm({
                title: 'Uyarý!',
                content: "Sisteminizde uzun süre iþlem yapýlmadýðý için kilit ekranýna yönleneceksiniz !",
                buttons:
                    {
                        Ok: {
                            text: "Kilitle",
                            btnClass: 'btn-blue',
                            action: function () { window.location.href = "/Admin/Login/Locked"; }
        ...
        }, (response.IntervalTime * 1000));
    }
};

function resetConnectionTime() {
    clearTimeout(myVar);
}
...
function firePriceFormatNumberOnly() {
    $('.eryaz-numeric-input-onlynumber').number(true, 0, ',', '');
...
function destroyPriceFormat() {
    $('.eryaz-numeric-input').number(false, 0);
}

function fireMainMenuActive() {
    var url = window.location.pathname,
        urlRegExp = new RegExp(url.replace(/\/$/, '') + "$");
    if (url === '' || url === '/') {
        $('#navigation a[href="/Admin"]').closest('li').addClass('active');
    }
    else {
        $('#navigation a').each(function () {
            ...
            if (urlRegExp.test(this.href.replace(/\/$/, ''))) {
                ...
                if ($(this).closest("li").closest("ul").closest("li").length > 0) {
                    $(this).closest("li").closest("ul").closest("li").addClass('open').addClass('active');
                    if ($(this).closest("li").closest("ul").closest("li").closest("ul").closest("li").length > 0) {
                        $(this).closest("li").closest("ul").closest("li").closest("ul").closest("li").addClass('open').addClass('active');
                    }
                }
            }
        });
